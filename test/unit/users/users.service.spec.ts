...
describe('UsersService', () => {
  ...
  describe('getOwnProfile', () => {
    it('should return user profile with profile data', async () => {
      ...
      expect(result).toEqual({
        ...
        sellerEnabledAt: mockUser.sellerEnabledAt,
    ...
    it('should throw NotFoundException if user not found', async () => {
      ...
      await expect(service.getOwnProfile(mockUser.id)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('getPublicProfile', () => {
    ...
    it('should throw NotFoundException for suspended users', async () => {
      mockPrismaService.user.findUnique.mockResolvedValue({
        ...mockUser,
        state: UserState.SUSPENDED,
      });

      await expect(service.getPublicProfile(mockUser.id)).rejects.toThrow(
        NotFoundException,
      );
    });

    it('should throw NotFoundException for banned users', async () => {
      mockPrismaService.user.findUnique.mockResolvedValue({
        ...mockUser,
  ...
  describe('updateProfile', () => {
    ...
    it('should throw ForbiddenException if user is not active', async () => {
      ...
      await expect(
        service.updateProfile(mockUser.id, { displayName: 'New Name' }),
  ...
  describe('enableSeller', () => {
    ...
    it('should throw BadRequestException if terms not accepted', async () => {
      await expect(
        service.enableSeller(mockUser.id, { acceptTerms: false }),
    ...
    it('should throw BadRequestException if user not active', async () => {
      mockPrismaService.user.findUnique.mockResolvedValue({
        ...mockUser,
    ...
    it('should throw BadRequestException if profile incomplete', async () => {
      mockPrismaService.user.findUnique.mockResolvedValue({
        ...mockUser,
        profile: null,
      });
  ...
  describe('getUserStats', () => {
    ...
    it('should throw NotFoundException if user not found', async () => {
      ...
      await expect(service.getUserStats(mockUser.id)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
