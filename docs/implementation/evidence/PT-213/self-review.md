# Evidencia — INVESTIGATION — La guarda de FORMA del contrato SSR-API (R-051)

**Fecha:** 2026-07-31 · **Rama:** `fix/PT-204-contrato-forma-de-lista`

Ver `docs/implementation/evidence/PT-204/self-review.md`, que documenta la corrida completa: el RED con
los siete consumidores acusados y **las tres veces que esta guarda midio otra cosa antes de servir**.

Hallazgo de la investigacion: `rutas-que-los-ssr-invocan.spec.ts` compara **rutas literales** —que el
endpoint exista—, no formas de respuesta. Estaba en verde con cuatro pantallas permanentemente vacias.
Es la respuesta a la pregunta que `S-013 §SIGUIENTE·3` dejo escrita.
