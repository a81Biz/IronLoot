...
export function toItems<T = unknown>(raw: unknown): ListView<T> {
  ...
  if (raw && typeof raw === 'object') {
    ...
    if (Array.isArray(obj.items)) return { items: obj.items as T[], total: Number(obj.total ?? obj.items.length), page: obj.page as number };
    if (Array.isArray(obj.data)) return { items: obj.data as T[], total: Number(obj.total ?? obj.data.length), page: obj.page as number };
  }
  return { items: [] };
}
