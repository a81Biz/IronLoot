/**
 * **Copia consciente del de BASE, no un descuido** (PT-194, decisión `D-7`).
 *
 * Son dos líneas de configuración, y los dos sitios SSR ya duplican `variable-obligatoria.ts` por la
 * misma razón: extraerlo a un paquete compartido añadiría un `file:` más al monorepo y una dependencia
 * de compilación entre sitios que hoy son independientes.
 *
 * **Lo que impide que diverjan no es el código compartido: es que ambos leen la MISMA variable de
 * entorno.** Si alguien cambia `JWT_ACCESS_EXPIRY`, cambian los dos. Duplicar la lectura de una
 * variable no crea dos fuentes de verdad — la fuente sigue siendo una, y está en el entorno.
 */
/**
...
export function duracionAMs(valor: string): number {
  ...
  if (!m) {
    ...
    throw new Error(
      `[CLIENT] Duración de sesión no reconocida: "${valor}". Formatos válidos: 900, 15m, 2h, 7d.`,
    );
  }
  const n = Number(m[1]);
