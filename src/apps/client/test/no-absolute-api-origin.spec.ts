...
describe("Las plantillas no incrustan direcciones de la API (PT-098)", () => {
  ...
  it("ninguna plantilla declara ya la constante `API` a partir de `apiUrl`", () => {
    ...
    const conApiUrl = ficheros.filter((f) =>
      /\{\{\s*apiUrl\s*\}\}/.test(readFileSync(f, "utf8")),
    );
    expect(conApiUrl.map((f) => f.replace(RAIZ, "views"))).toEqual([]);
  });

  it("la puja en vivo conecta al propio origen, no a un host externo", () => {
    // PT-096 movio este script de la plantilla a su propio fichero. La comprobacion es la
    // misma; cambia donde vive el codigo.
    const detalle = readFileSync(
      join(RAIZ, "..", "public", "js", "pages", "pages-auction-detail.js"),
      "utf8",
    );
