...
export class AppService {
  ...
  getStats() { return this.call('GET', '/admin/stats'); }
  getExtendedStats() { return this.call('GET', '/admin/dashboard/extended-stats'); }
  getRevenueByDay(days = 90) { return this.call('GET', `/admin/dashboard/revenue-by-day?days=${days}`); }
  getNewUsersByDay(days = 30) { return this.call('GET', `/admin/dashboard/users-by-day?days=${days}`); }
  ...
  updateUser(id: string, data: Record<string, unknown>) { return this.call('PATCH', `/admin/users/${id}`, data); }
  ...
  getCommissionsRecords(page = 1, status?: string) {
    const qs = new URLSearchParams({ page: String(page) });
    if (status) qs.set('status', status);
    return this.call('GET', `/admin/financial/commissions/records?${qs}`);
  }
  ...
  deleteCommissionConfig(id: string) { return this.call('DELETE', `/admin/financial/commissions/config/${id}`); }
  markCommissionCollected(id: string) { return this.call('PATCH', `/admin/financial/commissions/records/${id}/mark-collected`); }
  ...
  getReport(type: string, from?: string, to?: string) {
    ...
    if (from) qs.set('from', from);
    if (to) qs.set('to', to);
    return this.call('GET', `/admin/reports/${type}?${qs}`);
  }
  ...
  getRefunds(status?: string, page = 1) {
    const qs = new URLSearchParams({ page: String(page) });
    if (status) qs.set('status', status);
    return this.call('GET', `/admin/refunds?${qs}`);
  }
  ...
  getReconciliation(provider: string, dateFrom: string, dateTo: string) {
    return this.call('GET', `/admin/reconciliation?provider=${provider}&dateFrom=${dateFrom}&dateTo=${dateTo}`);
  }

  async exportReconciliation(provider: string, dateFrom: string, dateTo: string): Promise<Buffer | null> {
    try {
      const res = await fetch(`${this.apiUrl}/api/v1/admin/reconciliation/export?provider=${provider}&dateFrom=${dateFrom}&dateTo=${dateTo}`, {
  ...
  getAllSeoConfigs() { return this.call('GET', '/admin/seo'); }
  ...
  getAllCmsContent() { return this.call('GET', '/admin/cms'); }
  updateCmsContent(key: string, value: string, metadata?: Record<string, unknown>) {
    return this.call('PUT', `/admin/cms/${key}`, { value, ...metadata });
  }
  ...
  getAuditLogs(page = 1, filters: { userId?: string; module?: string; action?: string; from?: string; to?: string } = {}) {
    ...
    return this.call('GET', `/admin/audit-logs?${qs}`);
  }

  // ─── Moderation ────────────────────────────────────────────────────────────

  getModerationQueue(page = 1) { return this.call('GET', `/admin/moderation?page=${page}`); }
  approveModeration(id: string, adminUser: string) { return this.call('PATCH', `/admin/moderation/${id}/approve`, { adminUser }); }
  ...
  getKycSubmission(id: string) { return this.call('GET', `/admin/kyc/${id}`); }
  approveKyc(id: string, adminUser: string) { return this.call('PATCH', `/admin/kyc/${id}/approve`, { adminUser }); }
  rejectKyc(id: string, reason: string, adminUser: string) { return this.call('PATCH', `/admin/kyc/${id}/reject`, { reason, adminUser }); }
  requestKycCorrection(id: string, notes: string, adminUser: string) { return this.call('PATCH', `/admin/kyc/${id}/request-correction`, { notes, adminUser }); }
  ...
  generateCfdi(orderId: string) { return this.call('POST', `/admin/cfdi/${orderId}/generate`); }
  cancelCfdi(orderId: string) { return this.call('POST', `/admin/cfdi/${orderId}/cancel`); }

  // ─── Notifications ─────────────────────────────────────────────────────────

  getCampaigns(page = 1) { return this.call('GET', `/admin/notifications/campaigns?page=${page}`); }
