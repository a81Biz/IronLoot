...
export class AuctionsAdminController {
  ...
  async auctions(
    ...
    const result = await this.auctionsAdminService.getAuctions(
      Number(page),
      status,
    );
  ...
  async rejectAuction(
    ...
    await this.auctionsAdminService.rejectAuction(
      id,
      body.reason,
  ...
  async forceCloseAuction(@Param("id") id: string, @Req() req, @Res() res) {
    await this.auctionsAdminService.forceCloseAuction(
      id,
      req.session.adminUser,
    );
    return res.redirect(`/auctions/${id}`);
