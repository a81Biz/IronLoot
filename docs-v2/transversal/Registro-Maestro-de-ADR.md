...
| ADR-025 | **Idempotencia de webhooks por id de evento** (`processed_webhook_events`, unica `(provider, event_id)`) | Aceptada (parcial) | PayPal reentrega hasta 25 veces en 3 dias hasta recibir un 2xx; sin deduplicacion cada reintento acreditaria de nuevo. Reserva-luego-acredita: `WalletService.deposit()` abre su propia transaccion y no admite cliente externo, asi que la unicidad es el punto de serializacion y la reserva se libera si la acreditacion falla. **Parcial**: solo aplica a proveedores que informan `eventId` (hoy PayPal); MercadoPago sigue expuesto (TD-006). | `payments.service.ts:creditOnce`, `schema.prisma` | PT-076 |
