...
| RN-27 | Moneda MXN global; `Decimal` no `Float`. | ✅ `MXN` en las cinco tablas con `currency` — medido en la base el 2026-07-29. | `schema.prisma:306` | **AUD-008 corregido** |
...
| ID | Regla | Estado real | Evidencia | Hallazgo |
|---|---|---|---|---|
| RN-60 | Retención de logs de auditoría. | ✅ **Una sola retención** — `LOG_RETENTION_DAYS` (90 por defecto), declarada en el código como *«single authoritative retention»*. Los webhooks tienen su propio mínimo de 30 días, que es otra cosa. | `system-cleanup` | **AUD-018 corregido** |
...
| RN-64 | **Retención de liquidación (holdback).** El neto de una venta (bruto − comisión) entra a `wallet.pendingBalance` (NO disponible). Se libera cuando **`shipment.deliveredAt` + `SETTLEMENT_HOLDBACK_HOURS`** (72) **o** cuando vence la **ventana de disputa** desde la creación del pedido (`DISPUTE_WINDOW_DAYS`=14), lo que ocurra primero; asiento `SETTLEMENT_RELEASE`. Cron `EVERY_30_MINUTES`. | ✅ Cumple | `auction-scheduler.releaseMaturedSettlements`, `wallet.service.releaseSettlement` | **PT-071 · corregida PT-174** |
| RN-64b | **La recepción la confirma quien recibe.** El vendedor declara `SHIPPED`; **sólo el comprador** puede confirmar `DELIVERED` — el vendedor recibe **403**. La transición pasa por `OrderStateMachine`. | ✅ Cumple | `shipments.service.ts`, `35-cierre-y-liquidacion.js` QA-CL-07 | **PT-173/PT-174** |
